...
const defaultCookies = Cookies.withAttributes({
  path: '/',
  domain: import.meta.env.VITE_APP_Domain
})

export const setCookie = (str, data, obj) => {
  defaultCookies.set(str, data, { expires: 300, ...obj })
}
export const getCookie = str => {
  return defaultCookies.get(str)
}
export const clearAllCookies = () => {
  ...
  var num = 0
    for (var i = 0; i < cookies.length; i++) {
      ...
      defaultCookies.remove(name.trim())
      num += 1
    }
  if (num == cookies.length) {
    return true
  }
};
...
export const getUser = (params = { latest: false }) => {
  const userInfo = getStore('userInfo') || {}
  const key = Object.keys(userInfo)
  if (params.latest || !key.length) {
    return axios({
      host: 'centerApi',
      url: 'userAuth'
    }).then(res => {
      if (res.status) setStore('userInfo', res.data)
      return res
    })
  } else {
